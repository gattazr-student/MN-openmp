...
/* ========================================================================== */
/* Factorisation LU d'une petite matrice de doubles */
void factLU_sdMatr(t_sdMatr aMatrice, t_sdMatr aMatrL, t_sdMatr aMatrU){
	int wI,wJ,k,err;
	double pivot,coef;

	for(wI = 1; wI <= SMALLN_MATR; wI++){
		for(wJ = 1; wJ <= SMALLN_MATR; wJ++){
			aMatrU[wI][wJ] = aMatrice[wI][wJ];
		}
	}
	err = 1;
	k = 1;

	while (err==1 && k < SMALLN_MATR){
		pivot = aMatrU[k][k];
		if(pivot != 0){
			for(wI = k + 1; wI <= SMALLN_MATR; wI++){
...
/* Factorisation LU d'une petite matrice de floats */
...
/* ========================================================================== */
/* Gaxpy (Generalizef Ax plus y) pour les petits doubles */
void sdGaxpy(t_sdMatr aMatr, t_sdMatrVect aX, t_sdMatrVect aY, t_sdMatrVect aResult){
	int wI, wJ;
	int wSomme;
...
/* Gaxpy (Generalizef Ax plus y) pour les petits floats */
void sfGaxpy(t_sfMatr aMatr, t_sfMatrVect aX, t_sfMatrVect aY, t_sfMatrVect aResult){
	int wI, wJ;
	int wSomme;
	#pragma omp parallel shared(aMatr, aX, aY, aResult) private(wI, wJ, wSomme)
	{
...
/* ========================================================================== */
/* Fonction multiplication de deux grandes matrices carrées de même dimension en double */
...
/* ========================================================================== */
/* Fonction de lecture d'un matrice de type t_sdMatr */
...
/* Fonction de lecture d'un matrice de type t_sfMatr */
...
/* ========================================================================== */
/* Fonction d'écriture d'un matrice de type t_sdMatr */
...
/* Fonction d'écriture d'un matrice de type t_sfMatr */
...
/* ========================================================================== */
/* Fonction d'écriture d'un matrice de type t_sdMatrVect */
void write_sdMatrVect(t_sdMatrVect aMatrVect){
	int wI;
	for(wI = 0; wI < SMALLN_MATR ; wI++){
		fprintf(stdout, "%f\n", aMatrVect[wI]);
	}
...
/* Fonction d'écriture d'un matrice de type t_sdMatrVect */
void write_sfMatrVect(t_sfMatrVect aMatrVect){
	int wI;
	for(wI = 0; wI < SMALLN_MATR ; wI++){
		fprintf(stdout, "%f\n", aMatrVect[wI]);
	}
