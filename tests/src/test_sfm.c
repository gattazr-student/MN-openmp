...
t_sfMatr M1, M2, M3;
t_sfMatrVect Vect_x, Vect_r, Vect_y;

int verifie_valeur (t_sfMatr matrice, double valeur) {
	int i;
	int j;
	int erreur = 0;
	i=0;
...
int tout_ok (t_sfMatrVect vecteur, double valeur) {
	int i;
	int erreur = 0;
	i=0;
	while ((i<SMALLN_MATR)&&(!erreur)) {
...
void init (t_sfMatrVect tab, double valeur) {
	int i;
	int j;
...
int main(int argc, char *argv[])
{
	...
	read_sfMatr(M1);  /* LECTURE d'une matrice */
	if (verifie_valeur(M1,valeur1)) {
		fprintf(stderr, "lecture incorrecte\n");
		return 1;
	}
	write_sfMatr(M1); /* ECRITURE d'une matrice */
	...
	/*	void mScalaire_sfMatr(t_sfMatr aMatr1, double aScalaire, t_sfMatr aMatr2);	*/
	...
	mScalaire_sfMatr(M1,scalaire,M2);
	...
	/* 		typedef double t_sfMatrVect[SMALLN_MATR];		*/
	...
	init(Vect_y,valeur3);
	sfGaxpy(M1,Vect_x,Vect_y,Vect_r);
